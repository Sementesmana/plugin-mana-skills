...
description: Plataforma de comercialização das revendas parceiras da Sementes Maná LTDA — as revendas informam o volume já comercializado do que adquiriram e a Maná acompanha a velocidade de comercialização. Flask + PostgreSQL (schema comercializacao) no Railway, integrado ao Simple Agro via mana-simpleagro + cache data-lake-pg. Reaproveita a UI do agente-estoque mas SEM produção/qualidade/estoque físico. Núcleo: Volume Adquirido (pedidos SA), Comercializado (movimentos append-only) e Saldo. RBAC de 4 perfis (Admin Maná, Gestor Maná, Vendedor, Revenda), acesso resolvido SEMPRE por conjunto de cliente_sa (clientes_visiveis), trava de safra, drilldown Variedade→Revenda/Vendedor→Pedidos SA e auditoria. Use SEMPRE no agente-comercio-revendas — banco, endpoints, sincronização SA, permissões, painel, filtros, drilldown, edição de comercializado, controle de usuários. Também quando mencionar: comercialização de revendas, volume comercializado, volume adquirido, velocidade de venda da revenda, clientes_visiveis, trava de safra.
---
