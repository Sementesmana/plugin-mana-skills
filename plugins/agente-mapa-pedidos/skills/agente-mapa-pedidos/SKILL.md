...
description: Mapa de atuação comercial da Sementes Maná LTDA (painel N2, produção) — onde a Maná vende soja, por município. Flask + Jinja + Leaflet + Chart.js + Alpine no Railway, sem banco próprio - lê pedidos do Simple Agro pelo lake agro.lake_pedidos_raw do banco-mana (fonte única anti-452, mantida pelo agente-agro) com fallback SA ao vivo e cache em memória de 30min. Agrega por município (volume, faturamento, nº de produtores, cultivar dominante, top clientes), classifica o município pela faixa do MAIOR cliente (top >100 ton, médio 10-100, pequeno <10) e resolve coordenadas por JSON local + Nominatim automático com fila de pendentes no admin. Filtros multi-select de safra, UF, cultivar, status e vendedor, toggle TON/BAG e comparativo entre 2 safras com 6 comportamentos de cliente. LGPD - painel com senha, CPF/CNPJ mascarado em log e API, hash MD5 não-reversível. Use SEMPRE em tarefas do agente-mapa-pedidos — GeoJSON do mapa, agregação por município, coordenadas/Nominatim, faixas de cliente, comparativo de safras, filtros, cache SA. Também quando mencionar mapa comercial, /api/mapa, /api/mapa-comparativo, /api/pendentes, /api/coord, coords_municipios.json, lake_pedidos_raw, CAR V2, agente-mapa-comercial-production.
---
